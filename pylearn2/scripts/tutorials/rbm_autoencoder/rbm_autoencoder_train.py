...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print "usage : yaml_file_dataset yaml_file_hid units_layer1 units_layer2..."
        sys.exit(0)

    yaml_file_dataset = sys.argv[1]
    yaml_file_hid = sys.argv[2]
    nb_units = map(int, sys.argv[3:])

    yaml_files = [yaml_file_hid]  * (len(nb_units))
