...
def grid_plot(visualizations, nbrows=None, nbcols=None, random=False, imshow_options=None):
    """
    ...
    for row in xrange(nbrows):
        for col in xrange(nbcols):
            ...
            if ind >= visualizations.shape[0]:
                break
            plt.subplot(nbrows, nbcols, k + 1)
            plt.axis('off')
            plt.imshow(visualizations[ind], **imshow_options)
